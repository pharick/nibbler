...
{
    if (settings.fieldWidth < settings.startLength || settings.fieldHeight < settings.startLength)
    {
        throw SnakeException("Field size should be at least " +
            std::to_string(settings.startLength) + "x" + std::to_string(settings.startLength));
    }
    ...
    if (settings.startLength > settings.fieldWidth)
    {
